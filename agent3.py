...
# Default templates if DeepSeek fails
...
def run_nuclei(target, templates=None, rate_limit=50):
    ...
    if templates:
        ...
        if not valid_templates:
            logging.warning("No valid templates provided, using default templates: %s", DEFAULT_TEMPLATES)
            valid_templates = DEFAULT_TEMPLATES
...
def process_request(request):
    ...
    vulnerability_type = request.get("vulnerability_type", "http")  # Mặc định là "http"

    if not target:
        return {"type": "scan_response", "status": "error", "errors": ["Missing target"]}

    valid_templates = templates if templates else []
    if use_deepseek and not valid_templates:
        prompt = (
        ...
        if template_list:
            ...
            logging.warning("DeepSeek failed to suggest templates, using default templates: %s", DEFAULT_TEMPLATES)
    ...
    try:
        ...
        response = {
            ...
            "templates_used": valid_templates
        }
        return response
